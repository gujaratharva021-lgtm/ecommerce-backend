...
func SetupRoutes(router *gin.Engine) {
	...
	api := router.Group("/api/v1")
	{
		...
		auth := api.Group("/auth")
		{
			...
			auth.GET("/me", middleware.AuthMiddleware(), middleware.CustomerOnly(), handlers.Me)
			auth.PUT("/me", middleware.AuthMiddleware(), middleware.CustomerOnly(), handlers.UpdateProfile)
		}
		...
		products := api.Group("/products")
		{
			...
			products.POST("/:id/reviews", middleware.AuthMiddleware(), middleware.CustomerOnly(), handlers.UpsertReview)
			products.DELETE("/:id/reviews", middleware.AuthMiddleware(), middleware.CustomerOnly(), handlers.DeleteReview)
		}
		...
		// ---- Notification routes (protected, customer only) ----
		api.GET("/notifications", middleware.AuthMiddleware(), middleware.CustomerOnly(), handlers.GetMyNotifications)

		// ---- Wallet routes (protected, customer only) ----
		api.GET("/wallet", middleware.AuthMiddleware(), middleware.CustomerOnly(), handlers.GetWallet)
		api.GET("/returns", middleware.AuthMiddleware(), middleware.CustomerOnly(), handlers.GetMyReturns)
		...
		// ---- Address routes (protected, customer only) ----
		...
		// ---- Wishlist routes (protected, customer only) ----
		...
		// ---- Order routes (protected, customer only) ----
		...
		// ---- Coupon routes (protected, customer only) ----
