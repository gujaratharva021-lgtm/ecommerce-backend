...
if err := db.AutoMigrate(&models.User{}, &models.Address{}, &models.DeliveryPartner{}, &models.Order{}); err != nil {
...
t.Helper()
if err := database.DB.Exec("TRUNCATE TABLE orders, delivery_partners, addresses, users RESTART IDENTITY CASCADE").Error; err != nil {
t.Fatalf("failed to reset tables: %v", err)
}
}

// seedUserAndAddress creates a user and a delivery address at the given
// coordinates, returning the address ID for use on a test order.
